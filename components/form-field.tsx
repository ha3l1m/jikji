...
export function FormField({ label, error, theme = 'dark', id, ...props }: FormFieldProps) {
  const inputClass =
    theme === 'dark'
  ...
  const labelClass =
    theme === 'dark' ? 'text-sm font-medium text-white/70' : 'text-sm font-medium text-black/80';
  ...
  return (
    <div className="space-y-2">
      ...
      {error && (
        <p className={`text-xs mt-1 ${theme === 'dark' ? 'text-red-400' : 'text-red-500 text-sm'}`}>
          {error.message}
        </p>
      )}
    </div>
